...
- **会话索引 + Desktop 任务登记 + 项目信任**：导入后追加到 `session_index.jsonl`，并通过 Codex app-server 的 metadata-only `thread/resume` 登记桌面端任务（不发送消息、不追加对话）；还可将重绑后的实际工作目录注册为当前设备上的可信项目（修改前备份 `config.toml`）。完整退出并重启 Codex Desktop 后，对话会出现在对应项目下。
...
勾选对话（可按文件夹批量筛选）→ 导出 zip
                  ── 网盘 / QQ / 微信 ──►
                                               导入 zip（预览 → 路径重绑 → 写入 + 登记索引/Desktop/可信项目）
                                               完整退出并重启 Codex Desktop → 在对应项目查看
```
...
5. 保持“将导入会话的实际工作目录注册为 Codex 可信项目”勾选，然后点导入。工具会在首次修改前把 `config.toml` 备份为 `config.toml.codexrelay.bak`。
6. 导入完成后**完整退出 Codex Desktop（包括系统托盘中的进程）并重新启动**，再进入映射后的项目目录查看；也可复制结果页的 `codex resume <id>` 验证会话。
...
导入结果页需要同时确认“已登记到 session_index”“已通过 app-server 登记”和“已注册为可信项目”。然后**完整退出 Codex Desktop（包括系统托盘中的进程）并重新启动**；仅关闭项目页或刷新界面不足以完成项目归类。若任一状态失败，请先按结果中的具体错误处理。仍看不到时，可用 `codex resume <session_id>` 验证；手动 resume 也会触发桌面端登记。
