...
- **前端 GUI 面板没加**：目前参数只能通过 `import_bundle` / `import_bundles` IPC 调用传入。如果你在 UI 上需要"填旧路径→选新路径文件夹"的表单，后续在 `src/App.tsx` 里加即可，后端接口已就绪。
- **Codex SQLite 会话索引不处理**：和原版一样，导入后建议用 `codex resume <session_id>` 命令行续聊；新版 Codex 是否把导入会话显示在 UI 项目列表，取决于它自己的索引扫描，不在本工具控制范围。
- **shell_snapshot.sh 不导入**（沿用原设计，避免把 A 机环境变量带到 B 机）。
